  ...
  return (
    <div className="space-y-6">
      <ConfirmDialog
        ...
        description={`O investimento "${pendingDelete?.name || 'selecionado'}" será removido da sua carteira. Essa ação não pode ser desfeita.`}
      ...
      <div className="rounded-lg bg-slate-950 p-5 text-white shadow-xl shadow-slate-300/40 sm:p-7">
        ...
        <div>
            <p className="mb-2 text-xs font-semibold uppercase tracking-[0.18em] text-cyan-200">Carteira</p>
            <h1 className="text-2xl font-bold sm:text-3xl">Investimentos</h1>
            <p className="mt-2 max-w-xl text-sm leading-6 text-slate-300">Gerencie seus investimentos e acompanhe o desempenho com clareza.</p>
        </div>
        <button
          onClick={() => {
            setShowForm(true)
            ...
            className="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-white px-4 py-3 text-sm font-semibold text-slate-950 transition-all hover:-translate-y-0.5 hover:bg-cyan-50 sm:w-auto"
        >
      ...
      <div className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
        <select
          ...
          className="min-h-[44px] w-full rounded-lg border border-gray-300 px-4 py-2 text-sm focus:border-transparent focus:ring-2 focus:ring-gray-500 sm:w-auto"
        >
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/60 p-0 backdrop-blur-sm sm:items-center sm:p-4">
          <div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-t-2xl bg-white p-5 shadow-2xl sm:rounded-lg sm:p-6">
      ...
      {!investments || investments.length === 0 ? (
        ...
        <div className="grid gap-3 md:hidden">
          {investments.map((investment) => (
            <div key={investment.id} className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="truncate text-base font-bold text-slate-950">{investment.name}</p>
                  <div className="mt-1 flex flex-wrap items-center gap-2">
                    <span className="rounded-full bg-cyan-50 px-2 py-1 text-xs font-semibold text-cyan-800">
                      {getTypeLabel(investment.type)}
                    </span>
                    {investment.ticker && <span className="text-xs font-semibold text-slate-500">{investment.ticker}</span>}
                  </div>
                </div>
                <div className="flex flex-shrink-0 gap-1">
                  <button
                    ...
                    aria-label={`Editar investimento ${investment.name}`}
                  >
                    <Edit className="h-4 w-4" />
                  </button>
                  <button
                    onClick={() => handleDelete(investment.id, investment.name)}
                    className="flex h-10 w-10 items-center justify-center rounded-lg text-red-600 hover:bg-red-50 hover:text-red-800"
                    aria-label={`Excluir investimento ${investment.name}`}
                  >
              ...
              <div className="mt-4 grid grid-cols-2 gap-3 border-t border-slate-100 pt-4">
                ...
                <div>
                  ...
                  <p className={`mt-1 text-sm font-bold ${investment.profit_loss >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                    {investment.profit_loss >= 0 ? '+' : ''}
                    {investment.profit_loss.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                  </p>
                </div>
                <div>
                  <p className="text-xs font-medium text-slate-500">Rentabilidade</p>
                  <p className={`mt-1 text-sm font-bold ${investment.profit_loss_percentage >= 0 ? 'text-green-600' : 'text-red-600'}`}>
